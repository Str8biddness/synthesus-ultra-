...
The device manifest is now sealed as `aivm.device_manifest.v1`. It records the sorted mounted-device set plus the expected fingerprint table and carries a canonical `manifest_hash`. Restore verifies the manifest hash, the device set, and the fingerprint table before per-device replay checks run, so a validly resealed outer snapshot cannot silently drop a mounted CHAL partition or alter the expected fingerprint manifest without tripping admission.
...
Snapshots now include `replay_trace` metadata for the canonical AIVM tick audit stream. The record uses `aivm.snapshot_replay.v1` and stores the ordered tick steps, compact audit event details, canonical-sequence status, emit hashes, scheduler class, the sealed `device_manifest_hash`, a SHA-256 `events_hash`, and a canonical `record_hash` over the compact replay identity. Sensitive replay detail fields such as `input`, `user_input`, `prompt`, `query`, `intent`, `draft`, `response`, `content`, and `text` are stored as redacted SHA-256 identity records with lengths, not raw values. Restore verifies `events_hash`, `record_hash`, the replay trace's device-manifest binding, and the replay-detail scrub contract before admitting the snapshot, then exposes the sealed record on `NPC.snapshot_replay_trace` while the restored live audit stream remains limited to the new spawn/restore events.
