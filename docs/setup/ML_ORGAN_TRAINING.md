...
- Current `organ-triad-replay-v4` traces include CHAL accelerator frame metadata under `replay.chal`: `frameId`, `parentFrameId`, `chal://organs/<domain>/<organ>` device URI, `role="organ_accelerator"`, route, output reference, candidate references, selected candidate reference, critic feedback reference, and a shared-backbone contract. They also include a compact `replay.record` with schema `organ_training_replay.v1` and a SHA-256 `recordHash` over deterministic replay identity fields. This keeps organ traces bounded as CHAL accelerators rather than independent brains, exposes candidate-generation and critic-feedback interfaces to evaluation, and lets comparison artifacts detect replay drift without storing full candidate bodies.
- `tools/evaluate_organs.py` can now persist compact CHAL organ replay storage with `--replay-jsonl <path>` and validate it with `--replay-integrity-json <path> --fail-on-organ-replay-integrity`. These records use schema `synthesus.organ_replay_trace.v1`, preserve source replay hashes, CHAL frame identity, full shared-backbone contract identity, candidate refs, selected candidate refs, critic feedback refs, acceptance, and quality, and intentionally omit raw state/action/trajectory feature vectors. The integrity gate also verifies that `replay.record` and `replay.chal` mirror the same candidate refs, selected candidate, critic acceptance/quality, and shared-backbone contract before a compact storage record is admitted. The reusable storage contract is mirrored as `OrganReplayTrace` in `docs/openapi.yaml`, `docs/openapi.json`, and `docs/api_schema.json`; it documents the evaluator artifact and is not a `/api/v1/query` payload.
...
- If replay identity coverage drops below 100%, check that every `replay.record` still has schema `organ_training_replay.v1`, matching domain/organ/phase/device fields, candidate refs, selected candidate ref, accepted/quality fields, shared-backbone contract metadata, and a valid `recordHash`.
...
- If replay integrity fails with a record/CHAL mirror mismatch, check that the source compact record and CHAL accelerator frame were generated from the same candidate set, selected candidate, critic result, and shared-backbone contract.
...
- `tools/evaluate_organs.py` now reports CHAL accelerator frame coverage, replay identity coverage, and candidate/critic feedback coverage for current `organ-triad-replay-v4` traces, and can fail when those traces lack CHAL-bounded organ metadata, shared-backbone contract metadata, valid compact replay hashes, or record/CHAL mirror integrity for candidate and critic fields.
- `tools/selfImprove.ts` runs evaluation with `--min-replay-coverage 1.0 --min-replay-identity-coverage 1.0 --min-chal-accelerator-coverage 1.0 --min-candidate-critic-coverage 1.0 --min-scientific-consistency 1.0 --fail-missing-models --replay-jsonl tools/results/organ_training_replay_latest.jsonl --replay-integrity-json tools/results/organ_training_replay_integrity_latest.json --fail-on-organ-replay-integrity` so generated traces must remain replayable, hash-identifiable, CHAL-bounded, critic-visible, numerically valid, and storable as compact replay metadata.
