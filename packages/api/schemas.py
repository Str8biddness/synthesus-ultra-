...
class QueryResponse(BaseModel):
    ...
    debug: Optional[Dict[str, Any]] = Field(
        ...
        description=(
            ...
            "QuadBrainArbitration records when route=quad_brain_path and "
            "QuadBrainReplayRecord metadata for compact replay/storage traces, "
            "plus CHALMemoryWritebackResult records when the API attempts "
            "post-arbitration memory writeback. CGPU candidate-set trace "
            "records should also live here as the runtime wiring expands "
            "without changing the stable response envelope. "
