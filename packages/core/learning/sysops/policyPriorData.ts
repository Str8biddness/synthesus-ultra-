// learning/sysops/policyPriorData.ts
// Builds training dataset for SysOps PolicyPrior organ from Teacher/Trace logs

import { StateFeatures, ActionFeatures } from '../../amplification/features';
import { loadEntries, PlanningTraceEntry } from '../teacherTrace';

export async function buildSysOpsPolicyPriorDataset(from: Date, to: Date): Promise<{ inputs: { stateFeatures: StateFeatures; actionFeaturesList: ActionFeatures[] }[]; targets: number[][] }> {
